...
- `smoke_relay.py` — exemple de référence (et matériel réel en prod) : pilote le relais
  GPIO de la machine à fumée. Câblage sur **GPIO 17** (BCM).
- `.env.example`, `klixa-smoke-relay.service` — config et service systemd pour cet exemple.

## Machine à fumée — installation

```bash
sudo apt update && sudo apt install -y python3 python3-pip
mkdir -p /home/rpi_user/klixa-device-agent && cd /home/rpi_user/klixa-device-agent
# Copier klixa_device_agent.py, smoke_relay.py et requirements.txt ici
...
Le token se génère **depuis l'IHM du compagnon** (app Klixa Companion → page
« Appareils LAN » → Générer un token) : donne-lui l'identifiant que tu vas mettre dans
`DEVICE_ID` (ex. `smoke-machine`), le token n'est affiché qu'une seule fois, à copier
immédiatement.

### Configuration

Créer `/home/rpi_user/klixa-device-agent/.env` (voir `.env.example`) :
...
```bash
sudo cp klixa-smoke-relay.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now klixa-smoke-relay
journalctl -u klixa-smoke-relay -f
```

Si le script fonctionne, le device apparaît « connecté » dans le compagnon (page
« Appareils LAN ») puis, une fois la feature tenant `devices` activée côté Klixa
(Instance Admin), dans la console Klixa cloud (Paramètres → Compagnon → carte
« Appareils LAN »).
...
- Durée bornée à `[SMOKE_MIN_MS, SMOKE_MAX_MS]` (défaut 50–1500 ms) dans
  `smoke_relay.py` — le relais ne peut pas rester bloqué « on » via une commande.
